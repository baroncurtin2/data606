...
# histograms
hist(mdims$hgt, xlab = "Height", main = "Men's Height")
hist(fdims$hgt, xlab = "Height", main = "Female's Height")
```
  * The men's dataset is normally distributed and uni-modal. The female's dataset is skewed right and also unimodal.
...
2.  Based on the this plot, does it appear that the data follow a nearly normal 
    distribution?
    * The data does appear to follow a nearly normal distrbution. THere are only slight deviations, especially near the peak of the historgram.
...
3.  Make a normal probability plot of `sim_norm`.  Do all of the points fall on 
    ...
    data?
    * The points near the ends of the distribution do not fall on the line. Towards the middle of the distrubtion, the data falls closer to normal.
```{r 3}
qqnorm(sim_norm)
qqline(sim_norm)
```
...
4.  Does the normal probability plot for `fdims$hgt` look similar to the plots 
    ...
    * The plots for `fdims$hgt` are similar to the simulated plot in that all of the plots deviate from normal towards the ends of the line and are more consitent with normal distributions towards the middle of the line.

5.  Using the same technique, determine whether or not female weights appear to 
    come from a normal distribution.
```{r 5}
hist(fdims$wgt, xlab = "Weight", main = "Female's Weight", probability = TRUE)
...
6.  Write out two probability questions that you would like to answer; one 
    ...
    * What is the probability that a female's height is taller than 190 cm?
```{r 6a}
1 - pnorm(q = 190, mean = fhgtmean, sd = fhgtsd)
```
  * What is the probability that a female's weight is lower than 50 kg?
```{r 6b}
pnorm(q = 50, mean = fwgt_mean, sd = fwgt_sd)
```
...
-   Note that normal probability plots C and D have a slight stepwise pattern.  
    ...
    * Stepwise patterns are usually created based on variable types. 
