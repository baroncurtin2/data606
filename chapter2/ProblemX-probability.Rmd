...
1.  What does a streak length of 1 mean, i.e. how many hits and misses are in a 
  ...
  * A streak of length 1 means that there is one made shot before a miss. A streak of length 0 means there are no made shots before the next miss.
...
2.  Describe the distribution of Kobe's streak lengths from the 2009 NBA finals. 
  ...
  * The distribution of Kobe's streak lengths is skewed right. His typical streak was one made basket (if he was on a streak). His longest streak of baskets is 4 made baskets.
...
4.  What change needs to be made to the `sample` function so that it reflects a 
    ...
    `sim_basket`.
    *  Probabilities must be assigned to "H"(.45) and "M"(.55).
    ```{r four}
...
-   Describe the distribution of streak lengths. What is the typical streak 
    ...
    * The distribution is skewed right. The typical streak is 1 shot (for made baskets), otherwise the most frequent streak is 0 made baskets. The longest streak is 4 baskets.
...
-   If you were to run the simulation of the independent shooter a second time, 
    ...
    * Running the simulation of the independent shooter a second time should yield similar results to the first one with slight variations due to chance. As more and more simulations are run, you would still expect to see a 45% conversion chance. The streaks may be slightly different, but should not be wildly different because the probability of missing/making each shot is the same.
...
-   How does Kobe Bryant's distribution of streak lengths compare to the 
    ...
    * Kobe's distribution matches the simulation. Both streaks are skewed right and the most frequent streaks occur at the same datapoint (0 and 1). They also match in their longest streak. The hot hand model does not fit Kobe's shooting pattern because the simulation was based off of an independent shooting model. Each shot is not affected by the previous shot and since the distributions mirror each other reasonably, it is safe to assume that Kobe's shooting pattern does not fit the hot hand model.
