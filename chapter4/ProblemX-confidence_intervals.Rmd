...
hist(samp)
```
  * The distribution is uni-modal and skewed right
  * Based on the histogram, I'd determine the typical size to be around 1500. Since the distribution of the sample is skewed, I estimated the median to mean "typical"


2.  Would you expect another student's distribution to be identical to yours? 
Would you expect it to be similar? Why or why not?
  * I would not expect another student's distribution to be identical because a sample of size 60 is only 2% of the population, and different samples will have different centers
...
met for this to be true?
  ...
  * The sample mean must be normally distributed
...
4.  What does "95% confidence" mean? If you're not sure, see Section 4.2.2.
  * A 95% CI indicates that there is a 95% chance the population mean is within the sample's CI
...
interval capture this value?
  * Yes the CI of 1401.686 - 1625.947 does capture the population mean
...
population mean.
  * 95% of those intervals should capture the true population mean because of the chance that the sampling error skews the sample mean away from the population mean
...
left <- df %>%
  filter(lower_vector > popMean) %>%
  count
right <- df %>%
  filter(upper_vector < popMean) %>%
  count

# proportion inc popMean
1-(left + right)/count(df)
```
  * The proportion of 94% is almost equal to the confidence interval. This is most likely due to rounding and sampling errors. With more samples, the proportion should approach/reach 95%.


-   Pick a confidence level of your choosing, provided it is not 95%. What is 
    the appropriate critical value?
    * A 99% confidence interval's critical value is 2.58
...
1-(left + right)/count(df)
```
  * 98% of the samples include the true population mean, which is slight off from the 99% confidence interval most likely due to sampling error and rounding
