...
1.  What type of plot would you use to display the relationship between `runs` 
    ...
    * A scatterplot seems best suited to display the relationship between runs and another variables.
```{r}
...
```{r cor, eval=TRUE}
x <- cor(mlb11$runs, mlb11$at_bats) %>%
  print
```
...
2.  Looking at your plot from the previous exercise, describe the relationship 
    ...
    * There appears to be a positive relationship between runs and at bats
    * A R^2 of `r x` indicates a positive correlation but one that is not very strong
...
3.  Using `plot_ss`, choose a line that does a good job of minimizing the sum of
    ...
    * The smallest sum of squares that I got was 123,721.9.
...
\]
  * The slope tells that homeruns have a greater influence on runs than atbats
...
mlb11 %>%
  ...
  mutate(atbats_resid = abs(at_bats - 5578),
         runs_resid = abs(runs - runs(-2789.2429, 0.6305, 5578))) %>%
  arrange(atbats_resid) %>%
  filter(atbats_resid == min(atbats_resid))
```
  * This is an overestimation by 15 runs
...
6.  Is there any apparent pattern in the residuals plot? What does this indicate
    ...
    * Based on the scatterplot, most of the residuals are centered around 0, indicating linearity.
...
7.  Based on the histogram and the normal probability plot, does the nearly 
    ...
    * Based on the histogram and normal probability plot, the residuals appear to be normal.

*Constant variability*:

8.  Based on the plot in (1), does the constant variability condition appear to 
    be met?
    * The constant variability condition does appear to be met
...
```
  * There does appear to be a positive linear relationship between bat_avg and runs
...
-   How does this relationship compare to the relationship between `runs` and 
    ...
    tell?
    * bat_avg appears to predict runs better than at_bats because the R^2 is siginificantly higher than at_bats
```{r}
data_frame(atbats = c(summary(m1)$r.squared), bat_avg = c(summary(m3)$r.squared)) %>%
  mutate(diff = atbats - bat_avg)
```
...
```{r}
colNames <- mlb11 %>%
  select(-(1:2), -(new_onbase:new_obs)) %>%
  names()

rSquares <- colNames %>%
  map(.f = function(x) {
    form <- as.formula(str_c("runs ~ ", x, sep = ""))
    model <- lm(form, data = mlb11)
    df <- data_frame(variable = c(x), r2 = c(summary(model)$r.squared))
    return(df)
  }) %>%
  reduce(.f = function(df, x) {
    bind_rows(df, x)
  }) 
...
```{r}
new_obs_model <- lm(runs ~ new_obs, data = mlb11)

# linearity
ggplot(mlb11, aes(x = new_obs, y = new_obs_model$residuals)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = 3)
```
  * The relationship looks linear but the relationship does not indicate any curvatures or abnormalities
  
```{r}
hist(new_obs_model$residuals)
```
  * The residuals are approximately normal based on the histogram

```{r}
qqnorm(new_obs_model$residuals)
qqline(new_obs_model$residuals)
```
  * The distribution very closely mirrors that of a straight line and further confirms the normality of the residual data
  * The scatter plot also seems to indicate that constant variability has been met
