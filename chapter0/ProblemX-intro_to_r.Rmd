...
```{r}
s <- "The overall trend is there is a positive relationship between girls that are baptized and time. There was a dip for a number of years, but the trend has recovered and has continued to increase since around 1660." %>%
  print
```
...
```{r}
s <- "Arbuthnot's data contains 82 rows and 3 columns. Present's data contains 63 rows and 3 columns." %>%
  print
```
...
s <- "There is still a greater proportion of boys being born to girls, however that trend is decreasing with time rather than being cyclical."
```
