...
datatypes <- sapply(cdc, function(x) {
...
```{r}
s <- "This mosaic plot reveals that more men are smokers and more women are non-smokers." %>%
  print
```
...
```{r}
# age would be a good variable to analyze against BMI. One would expect that as people grow older, the BMI should rise with age.
# The rise would push the median for a healthy individual as they grow older
...
s <- "There is a quadratic correlation between Weight and Desired Weight. At lower weights, people generally want to be heavier (most likely men), and at higher weights, people generally want to be lower."
```
...
```{r}
s <- "Wdiff is a discrete variable. Whenever wdiff is 0, it means the person is happy at their current weight. A negative number indicates that they are unhappy and want to lose weight. A positive number indicates the person wants to gain weight." %>%
  print
```
...
```{r}
summary(new_col$wdiff)
s <- "The distribution is primarily negative. This tells us that generally people are unhappy how heavy they are and want to lose weight."
```

-   Using numerical summaries and a side-by-side box plot, determine if men tend
    to view their weight differently than women.
```{r}
boxplot(new_col$wdiff ~ new_col$gender)
```
...
```{r}
avg <- mean(new_col$weight)
std <- sd(new_col$weight)
low <- avg - std
high <- avg + std

proportion <- cdc %>%
  filter(weight >= low, weight <= high) %>%
  count
