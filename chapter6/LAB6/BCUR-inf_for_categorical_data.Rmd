...
1.  In the first paragraph, several key findings are reported. Do these 
    ...
    * The percentaegs reported appear to be calculated from the actual population parameters.
...
2.  The title of the report is "Global Index of Religiosity and Atheism". To
    ...
    assumption?
    * We must assume the findings were a derived from a simple random sample, the groups are independent of each other, the sample size is large enough, and the sample is satisfactory for inference. The report appears to indicate that the sample meets the criteria for inference.
...
3.  What does each row of Table 6 correspond to? What does each row of 
    ...
    * Each row of Table 6 corresponds to a different responder of the survey.
...
4.  Using the command below, create a new dataframe called `us12` that contains
    ...
    * The proportion of atheist response does agree as it is about ~5%.
...
beliefs <- us12 %>%
  group_by(response) %>%
  summarise(belief = n()) %>%
  mutate(percent = belief / nrow(us12))
...
5.  Write out the conditions for inference to construct a 95% confidence
    ...
    * Success-failure conditions require np >= 10 and n(1 - p) >= 10. np = 1002 \* .05 = 50.1 and n(1-p) = 1002 \* .95 = 951.9
...
inference(brazil$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
bse <- .0022
bme <- z * bse
...
france <- atheism %>%
  filter(nationality == 'France' & year == 2012)
inference(france$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
fse <- .011
fme <- z * fse

data_frame(country = c('Brazil', 'France'), error = c(bme, fme))
```
...
8.  Describe the relationship between `p` and `me`.
  * The relationship between `p` and `me` is parabolic with a maximum at .5. The closer the population proportion is to .5, the higher the margin of error.
...
9.  Describe the sampling distribution of sample proportions at $n = 1040$ and 
    ...
    *Hint:* Remember that R has functions such as `mean` to calculate summary
    statistics.
    * The sampling distribution is apporximately normal, unimodal, and not skewed to one side or the other. The median and mean of the distribution are near .1, which is also the population proportion.
...
```
  * Australia's conditions meet the conditions for inference
  * Ecuador's conditions do not meet the conditions for inference
...
spain2005 <- atheism %>%
  ...
  group_by(response) %>%
  summarise(belief = n()) %>%
  mutate(percent = belief / sum(belief))
...
spain <- atheism %>%
  ...
  inference(x = .$year, y = .$response, est = "proportion", type = "ht", null = 0, alternative = "twosided", method = "theoretical", success = "atheist")
```
...
us <- atheism %>%
  ...
  inference(x = .$year, y = .$response, est = "proportion", type = "ht", null = 0, alternative = "twosided", method = "theoretical", success = "atheist")
```
  * The p value is zero and this the NULL hypothesis is rejected. There is convincing evidence that the US has seen a change in its atheism index.
...
-   If in fact there has been no change in the atheism index in the countries 
    ...
    * There are 39 countries listed in Table 4. 5% of 39 is `r .05 * 39`.
